function [ node_accuracy ] = randomFeatures( x,y, len, alg4_path, K )
%NODECLASSIFY Summary of this function goes here
%   Detailed explanation goes here

[no_row,no_column]=size(x);
fid = fopen(alg4_path, 'w+');
variance = 0;
positive = 0;
negative = 0;
true_positive = 0;
...
true_negative = 0;
false_negative = 0;
len = floor(len * no_column);
r = unidrnd(no_column - len);
[train_index] = crossvalind('Kfold',no_row,K);
new_x = next_choosed_column(x, r,len-1);  
for i=1:K
    Train_x=new_x(train_index~=i,:);
    Train_y=y(train_index~=i);
    
    Test_x=new_x(train_index==i,:);
